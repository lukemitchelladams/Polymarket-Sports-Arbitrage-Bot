#!/usr/bin/env python3
"""
PolyArb v9 — ARB-FIRST SPORTS TRADING ENGINE
═══════════════════════════════════════════════════════════════
Based on RN1's actual strategy ($5.2M lifetime):
  ...
  - Non-arb entries spray NO across sports markets

TWO-ENGINE ARCHITECTURE:
  ENGINE 1: ARB SCANNER (PRIMARY)
    - Batch fetch ALL prices (NO+YES) in parallel (12x speedup)
    - WebSocket real-time streaming when available (140x speedup)
    - Any market where YES_ask + NO_ask < 0.99 → buy BOTH sides
    - 10% bankroll per arb market, rescan every cycle to keep building
    - Arbs ALWAYS increase ROI → always valid
    
  ENGINE 2: ROI OPTIMIZER (SECONDARY)
    - For existing positions: "does this action increase my ROI?"
    - Winner add (up 25%+), dip buy (down 15%+), loss cut (down 35%+)
    - Arb-locked positions: HOLD to resolution (profit guaranteed)

Usage:
  python3.11 arb_v8.py --bankroll 1000 --fresh                # dry run
  python3.11 arb_v8.py --bankroll 1000 --fresh --live          # real money
  python3.11 arb_v8.py --bankroll 1000 --fresh --scan          # scan only
"""
...
WINNING_THRESHOLD  = 0.70    # side at 70%+ = hammer it
DYING_THRESHOLD    = 0.30    # side at 30%- = sell it
ABS_DROP_CUT       = 0.20    # cut if price drops 20¢+ from entry (regardless of %) 

# ─── MOMENTUM REBALANCE ("House Mode") ───
REBAL_TRIGGER      = 0.08    # rebalance when NO price moves 8¢+ from last rebalance
REBAL_SELL_FRAC    = 0.25    # sell 25% of losing side per rebalance
REBAL_COOLDOWN     = 45      # seconds between rebalances (avoid whipsaw)

# ─── Entry (Pillar 1: SPRAY) ───
# Exact median sizes from 200K trade analysis:
SPORT_SIZE = {
    "nba": 21.43, "mlb": 20.01, "nfl": 19.49, "tennis": 18.70,
    "college": 14.61, "esports": 10.25, "soccer": 8.71, "nhl": 8.84,
    "mma": 10.0, "unknown_sport": 8.63,
}
TYPE_SIZE = {
    "match": 20.44, "win": 10.65, "esports": 10.50, "ou": 4.43,
    "draw": 8.57, "spread": 6.47, "btts": 7.54, "other": 10.0,
}
NO_MIN_PRICE    = 0.05       # allow cheap NO for arb opportunities
NO_MAX_PRICE    = 0.97       # allow expensive NO for arb locking
DAILY_NEW_TARGET = 100       # target new markets per day

# ─── Sport-specific management ───
# ─── Market Filtering: STRICT SPORTS ONLY ───
# RN1 ONLY trades sports event markets that resolve within ~24h.
# Pattern: "X vs Y", "Will X win on DATE", "X vs Y: O/U N", "draw", "BTTS"
# NEVER: crypto, politics, tech, futures, season-long bets
MAX_DAYS_OUT = 7  # skip markets resolving more than 7 days from now

# ─── Live Management (Pillar 2: MANAGE) ───
MONITOR_INTERVAL = 3         # seconds between price checks per position
DIP_BUY_THRESH   = -0.15    # buy more if NO dropped 15%+ from our avg (dip buy)
SPIKE_WARN       = 0.40      # flag if NO rose 40%+ from avg (potential profit-take)

# ─── Loss Management (Pillar 3: CUT + HEDGE) ───
LOSS_CUT_PCT     = -0.35     # sell 80% if NO drops 35%+ below avg cost
LOSS_SELL_FRAC   = 1.00      # sell 100% when cutting — no zombie positions
HEDGE_TRIGGER    = -0.23     # start hedging other side at 23% loss
HEDGE_SIZE_FRAC  = 0.50      # hedge = 50% of main position cost

# ─── Winner Management (Pillar 4: HOLD + DOUBLE DOWN) ───
# NO PROFIT-TAKING! 92% of positions hold to $1 resolution.
# When winning → BUY MORE. That's the actual RN1 behavior.
WINNER_ADD_PCT   = 0.25      # add when NO rose 25%+ from avg (position winning)
WINNER_ADD_MULT  = 1.3       # buy 130% of normal add size on confirmed winners
...
class Position:
    ...
    @property
    def should_cut(self):
        """Should we cut this loss? Two triggers:
        1. % drop from avg (LOSS_CUT_PCT, default -35%)
        2. Absolute drop of 20¢+ from entry (catches 65¢→45¢ type moves)
        With limit orders, avg_price ≈ mid-price so P&L is accurate
        from the start. Just a 2-min cooldown to let orders settle."""
        if self.shares <= 0: return False
        # 2-minute cooldown after entry (let limit orders settle)
        if self.age_hours * 3600 < 120: return False
        # Use bid-based P&L for actual sell decisions
        pnl = self.pnl_pct_bid
        # Only cut if loss exceeds the entry spread (real movement, not just spread)
        effective_cut = LOSS_CUT_PCT - self.spread_at_entry
        
        # Trigger 1: % drop from avg
        if pnl <= effective_cut: return True
        
        # Trigger 2: absolute price drop (e.g. 65¢ → 45¢ = 20¢ drop)
        if self.current_bid and self.avg_price > 0:
            abs_drop = self.avg_price - self.current_bid
            if abs_drop >= ABS_DROP_CUT: return True
        
        return False
    
    @property
    def should_hedge(self):
        """Should we hedge? Two triggers:
        1. % drop between -23% and -35% (spread-aware)
        2. Absolute drop 12-20¢ from entry"""
        if self.shares <= 0: return False
        if self.age_hours * 3600 < 120: return False
        pnl = self.pnl_pct_bid
        effective_hedge = HEDGE_TRIGGER - self.spread_at_entry
        
        # % trigger: between hedge and cut thresholds
        if effective_hedge <= pnl <= (LOSS_CUT_PCT - self.spread_at_entry):
            return True
        
        # Absolute trigger: 12¢+ drop but less than cut threshold (20¢)
        if self.current_bid and self.avg_price > 0:
            abs_drop = self.avg_price - self.current_bid
            if 0.12 <= abs_drop < ABS_DROP_CUT:
                return True
        
        return False
    
    @property
    def should_add_winner(self):
        """Position winning — double down! (NO rose 25%+)"""
        return self.pnl_pct >= WINNER_ADD_PCT and self.shares > 0
    
    @property
    def should_add(self):
        """Should we buy the dip? (NO dropped 15%+ beyond entry spread = real dip)
        NEVER dip-buy a position that was already cut — don't throw good money after bad."""
        if self.shares <= 0: return False
        if self.n_sells > 0: return False  # was cut — don't rebuy
        if time.time() - self.last_sell < 300: return False  # 5min cooldown after any sell
        pnl = self.pnl_pct
        effective_dip = DIP_BUY_THRESH - self.spread_at_entry
        effective_cut = LOSS_CUT_PCT - self.spread_at_entry
        return pnl <= effective_dip and pnl > effective_cut
    
    @property
    def should_hedge_or_cut(self):
        """Past the hedge threshold — needs action (% or absolute)."""
        if self.shares <= 0: return False
        pnl = self.pnl_pct
        effective_hedge = HEDGE_TRIGGER - self.spread_at_entry
        if pnl <= effective_hedge: return True
        # Absolute: 12¢+ drop
        if self.current_bid and self.avg_price > 0:
            if (self.avg_price - self.current_bid) >= 0.12: return True
        return False
...
class Portfolio:
    ...
    def save(self, path="portfolio_v8.json"):
        data = {}
    ...
    def load(self, path="portfolio_v8.json"):
...
# ═══════════════════════════════════════════════════════════════
# SIZING — calibrated from RN1 data
# ═══════════════════════════════════════════════════════════════
def entry_size(market_type="match"):
    """First buy size, calibrated from RN1's exact sizing data."""
    base = TYPE_SIZE.get(market_type, 10.0)
    scale = BANKROLL / 1000  # scale relative to $1K base
    size = base * scale
    return round(min(size, BANKROLL * MAX_PER_MARKET * 0.5), 2)

def add_size(market_type="match"):
    """Add buy size (~83% of entry, from RN1 data: $10.67/$12.88)."""
    return round(entry_size(market_type) * 0.83, 2)

# ═══════════════════════════════════════════════════════════════
# PILLAR 1: SPRAY — Discover and enter new markets
# ═══════════════════════════════════════════════════════════════
...
def arb_engine(all_markets):
    """DEDICATED ARB ENGINE — runs EVERY cycle (~100ms), ZERO sleeps on arb path.
    
    This is the CORE profit generator, modeled on RN1's behavior:
    RN1 hammers 50-150+ trades per market, eating through entire order book.
    
    PHASE 1: BUILD EXISTING ARBS — re-buy more matched pairs on positions we own
      - For each position with YES token: is combined < threshold?
      - If arb-locked: buy MORE matched pairs (eat the book)
      - If not locked: buy YES to LOCK it
      - NO SLEEP between trades — maximum speed
    
    PHASE 2: SCAN ALL MARKETS for new arb opportunities
      - Batch fetch all NO+YES prices (WS cache first, parallel HTTP fallback)
      - Any combined < threshold → enter immediately
      - Subscribe new tokens to WS for next cycle
            ...
            # ── PATH B: ALREADY LOCKED — eat more of the book ──
                ...
                if pair_cost < 1.0:  # still profitable at current prices
                    ...
                    if max_pairs >= 1:
                        ...
                        batch_profit = max_pairs - (batch_no_cost + batch_yes_cost)
                        
                        if batch_profit > 0.10 and batch_no_cost >= 1 and rate_ok():
...
def spray_entry(markets):
    """ENGINE 2: NO-SPRAY for non-arb markets (secondary to arb_engine).
    
    Arb detection is handled by arb_engine() which runs every cycle.
    This function ONLY does normal NO spray on markets where no arb exists.
    Uses batch parallel fetch for speed.
    """
    entered = 0
    
    # Batch fetch all NO prices in parallel
    tokens = [m["tok_no"] for m in markets]
    cache = smart_batch_fetch(tokens)
    
    for m in markets:
        mid = m["mid"]
        if portfolio.has(mid): continue
        if not rate_ok(is_new=True): break
        
        no_ask, no_depth, no_bid, _ = cache.get(m["tok_no"], (None, 0, None, 0))
        if not no_ask or no_ask <= 0: continue
        
        # Normal spray: price filter applies
        if no_ask < NO_MIN_PRICE or no_ask > NO_MAX_PRICE: continue
        if no_depth < 5: continue
        
        # Spread guard
        if no_bid and no_ask > 0 and (no_ask - no_bid) / no_ask > 0.30:
            continue
        
        room = portfolio.room_for(mid, m["group"])
        if room < 2: continue
        
        size = min(entry_size(m["typ"]), room)
        if size < 1: continue
        if size / no_ask < 5: continue
        
        label = f"ENTRY|{m['typ']}|{m['title'][:30]}"
        oid, shares = exec_buy(m["tok_no"], no_ask, size, label,
                               mode="aggressive", bid=no_bid, ask=no_ask)
        if not oid: continue
        
        pos = Position(mid, m["title"], m["typ"], m["tok_no"],
                       m.get("tok_yes", ""), m["group"])
        pos.record_buy(no_ask, size)
        pos.current_ask = no_ask
        pos.current_bid = no_bid
        if no_bid and no_ask > 0:
            pos.spread_at_entry = (no_ask - no_bid) / no_ask
        portfolio.add_position(pos)
        portfolio.n_entries += 1
        portfolio.total_cost += size
        record_order(is_new=True)
        entered += 1
    
    if entered > 0:
        log.info(f"  🎯 Sprayed {entered} new NO positions")
    return entered
...
# ═══════════════════════════════════════════════════════════════
# PILLAR 2+3+4+5: MANAGE — Arb first, then manage positions
# ═══════════════════════════════════════════════════════════════
def manage_positions():
    """ENGINE 3: MANAGE non-arb positions only.
    
    Arb scanning/building is handled by arb_engine() every cycle.
    This function handles: winner add, dip buy, loss cut, hedge.
    """
    now = time.time()
    actions = {"winner_add": 0, "loss_cut": 0, "hedge": 0, "dip_buy": 0, "checked": 0}
    
    # Only manage NON-ARB positions (arb_engine handles arb-locked ones)
    active = [(mid, pos) for mid, pos in portfolio.positions.items() 
              if pos.shares > 0 and not pos.is_arb_locked and not pos.is_two_sided
              and now - pos.last_check >= MONITOR_INTERVAL]
    if not active:
        return actions
    
    # Batch fetch all prices in parallel
    tokens = [pos.tok_no for _, pos in active]
    yes_tokens = [pos.tok_yes for _, pos in active if pos.tok_yes]
    price_cache = smart_batch_fetch(tokens + yes_tokens)
    
    for mid, pos in active:
        pos.last_check = now
        actions["checked"] += 1
        
        ask, _, bid, _ = price_cache.get(pos.tok_no, (None, 0, None, 0))
        pos.current_ask = ask
        pos.current_bid = bid
        if not bid or bid <= 0: continue
        
        if bid > pos.peak_price: pos.peak_price = bid
        if bid < pos.trough_price: pos.trough_price = bid
        
        pnl = pos.pnl_pct
        
        # Get YES price for hedge decisions
        yes_ask = None
        yes_bid = None
        if pos.tok_yes:
            yes_ask, _, yes_bid, _ = price_cache.get(pos.tok_yes, (None, 0, None, 0))
            pos.current_yes_ask = yes_ask
            pos.current_yes_bid = yes_bid
        
        # ── WINNING: Double down ──
        if pos.should_add_winner:
            if now - pos.last_buy > 45:
                room = portfolio.room_for(mid, pos.group_key)
                size = min(add_size(pos.typ) * WINNER_ADD_MULT, room)
                if size >= 1 and rate_ok():
                    limit_price = calc_limit_price(bid, ask, mode="limit")
                    if limit_price:
                        label = f"WINNER+|+{pnl*100:.0f}%|{pos.short()}"
                        oid, shares = exec_buy(pos.tok_no, limit_price, size, label,
                                               mode="limit", bid=bid, ask=ask)
                        if oid and shares > 0:
                            pos.record_buy(limit_price, size)
                            portfolio.total_cost += size
                            portfolio.n_dip_buys += 1
                            record_order()
                            actions["winner_add"] += 1
        
        # ── LOSS MANAGEMENT ──
        abs_drop = (pos.avg_price - (bid or 0)) if pos.avg_price > 0 and bid else 0
        needs_loss_mgmt = pnl <= HEDGE_TRIGGER or abs_drop >= 0.12
        if needs_loss_mgmt:
            has_hedge = pos.yes_shares > 0
            
            if pos.should_cut and (has_hedge or not pos.tok_yes or pnl <= LOSS_CUT_PCT * 1.5 or abs_drop >= ABS_DROP_CUT):
                sell_shares = round(pos.shares * LOSS_SELL_FRAC, 1)
                if sell_shares >= 1 and rate_ok():
                    sell_price = calc_limit_price(bid, ask, mode="bid")
                    if sell_price:
                        label = f"CUT|{pnl*100:.0f}%|{pos.short()}"
                        oid, rev = exec_sell(pos.tok_no, sell_price, sell_shares,
                                             label, mode="limit")
                        if oid == "NO_BALANCE":
                            pos.shares = 0; pos.cost = 0; pos.active = False
                        elif oid:
                            pos.record_sell(sell_price, sell_shares, rev)
                            portfolio.total_realized += rev
                            portfolio.n_loss_cuts += 1
                            record_order()
                            actions["loss_cut"] += 1
            
            elif not has_hedge and pos.tok_yes and pos.should_hedge:
                hedge_size = min(pos.cost * HEDGE_SIZE_FRAC,
                                portfolio.room_for(mid, pos.group_key))
                if hedge_size >= 1 and rate_ok() and yes_ask and 0.05 < yes_ask < 0.95:
                    hedge_price = calc_limit_price(yes_bid, yes_ask, mode="limit")
                    if hedge_price:
                        label = f"HEDGE|{pnl*100:.0f}%|{pos.short()}"
                        oid, shares = exec_buy(pos.tok_yes, hedge_price, hedge_size,
                                               label, mode="limit", bid=yes_bid, ask=yes_ask)
                        if oid and shares > 0:
                            pos.record_yes_buy(hedge_price, hedge_size)
                            portfolio.total_cost += hedge_size
                            record_order()
                            actions["hedge"] += 1
        
        # ── DIP BUY ──
        elif pos.should_add and ask:
            if now - pos.last_buy > 60:
                room = portfolio.room_for(mid, pos.group_key)
                size = min(add_size(pos.typ), room)
                if size >= 1 and rate_ok():
                    limit_price = calc_limit_price(bid, ask, mode="limit")
                    if limit_price:
                        label = f"DIP|{pnl*100:.0f}%|{pos.short()}"
                        oid, shares = exec_buy(pos.tok_no, limit_price, size,
                                               label, mode="limit", bid=bid, ask=ask)
                        if oid and shares > 0:
                            pos.record_buy(limit_price, size)
                            portfolio.total_cost += size
                            portfolio.n_dip_buys += 1
    ...
    if actions["checked"] > 0:
        log.info(f"  📊 Managed {actions['checked']}: "
                 f"win+={actions['winner_add']} cut={actions['loss_cut']} "
                 f"hedge={actions['hedge']} dip={actions['dip_buy']}")
    return actions
...
def dashboard():
    ...
    print(f"\033[1;96m║  PolyArb v8 — FULL RN1 STRATEGY   {now_s}  {'DRY' if DRY_RUN else 'LIVE':>4s}  ⏱ {runtime:.0f}m       ║\033[0m")
    print(f"\033[1;96m╚═══════════════════════════════════════════════════════════════════════════╝\033[0m")
    ...
    needs_cut = [p for p in open_pos if p.should_cut]
    needs_profit = [p for p in open_pos if p.should_add_winner]
    needs_add = [p for p in open_pos if p.should_add]
    ...
    if needs_cut or needs_profit or needs_add or near_arbs:
        print(f"\n  \033[1mACTION QUEUE:\033[0m")
        for p, comb in near_arbs[:3]:
            gap = (comb - ARB_LOCK_THRESHOLD) * 100
            print(f"  \033[96m  🔒 NEAR ARB   {p.short():<30s} "
                  f"combined={comb:.3f} ({gap:+.1f}% to lock)\033[0m")
        for p in needs_profit[:3]:
            print(f"  \033[92m  ↑ ADD WINNER  {p.short():<30s} +{p.pnl_pct*100:.0f}%\033[0m")
        for p in needs_cut[:3]:
            print(f"  \033[91m  ↓ CUT LOSS    {p.short():<30s} {p.pnl_pct*100:.0f}%\033[0m")
        for p in needs_add[:3]:
            print(f"  \033[93m  + DIP BUY     {p.short():<30s} {p.pnl_pct*100:.0f}%\033[0m")
...
def main():
    ...
    global BANKROLL, DRY_RUN, MONITOR_INTERVAL
    BANKROLL = args.bankroll
    if args.live: DRY_RUN = False
    if args.turbo: MONITOR_INTERVAL = 3  # --turbo is legacy, already fast by default
    
    if args.fresh:
        for f in ["portfolio_v8.json", "ledger.json"]:
    ...
    log.info("=" * 60)
    log.info(f"PolyArb v9 — ARB-FIRST ENGINE")
    ...
    log.info(f"  Sizing: match=${entry_size('match'):.2f} esports=${entry_size('esports'):.2f} "
             f"win=${entry_size('win'):.2f} ou=${entry_size('ou'):.2f} draw=${entry_size('draw'):.2f}")
    log.info(f"  Orders: AGGRESSIVE at ask (arbs), LIMIT at mid (entries/adds)")
    log.info("=" * 60)
    ...
    if all_markets:
        ...
        log.warning("  ⚠ NO SPORTS MARKETS FOUND — check logs/v8_*.log for details")
    ...
    while running:
        ...
        # ─── DISCOVER: refresh market list every 2 min ───
        ...
        # ─── ENGINE 1: ARB ENGINE — runs EVERY cycle (~50ms) ───
        ...
        # ─── ENGINE 2: NO SPRAY — every 15s, secondary to arbs ───
        if now - last_spray >= 15 and all_markets and not args.scan:
            batch_size = min(30, len(all_markets))
            batch = all_markets[scan_offset:scan_offset + batch_size]
            scan_offset = (scan_offset + batch_size) % max(len(all_markets), 1)
            spray_entry(batch)
            last_spray = now
        
        # ─── ENGINE 3: MANAGE non-arb positions every 5s ───
        if now - last_manage >= 5 and not args.scan:
            manage_positions()
            last_manage = now
        
        # ─── SCAN MODE: batch price-check ───
        if args.scan and all_markets:
            batch_size = min(30, len(all_markets))
            batch = all_markets[scan_offset:scan_offset + batch_size]
            scan_offset = (scan_offset + batch_size) % max(len(all_markets), 1)
            tokens = [m["tok_no"] for m in batch]
            cache = smart_batch_fetch(tokens)
            for m in batch:
                ask, _, bid, _ = cache.get(m["tok_no"], (None, 0, None, 0))
                if ask and 0.01 <= ask <= 0.99:
                    if not portfolio.has(m["mid"]):
                        p = Position(m["mid"], m["title"], m["typ"], m["tok_no"])
                        p.current_ask = ask
                        p.current_bid = bid

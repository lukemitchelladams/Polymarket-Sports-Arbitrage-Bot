...
Usage:
  python3.11 watch_v9.py                # default 5s refresh
  python3.11 watch_v9.py --refresh 3    # faster
  python3.11 watch_v9.py --once         # print once and exit
"""
...
PORTFOLIO  = os.path.join(SCRIPT_DIR, "portfolio_v9.json")
...
def find_log():
    ...
    fs = glob.glob(os.path.join(d, "v9_*.log"))
...
def main():
    ...
    while True:
        data = read_portfolio()
        if not data:
            print(f"{RD}No portfolio_v9.json found. Is the bot running?{R}")
            if once: return
